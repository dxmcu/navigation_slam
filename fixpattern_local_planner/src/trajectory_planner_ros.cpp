...
void FixPatternTrajectoryPlannerROS::initialize(std::string name, tf::TransformListener* tf, costmap_2d::Costmap2DROS* costmap_ros) {
  if (!isInitialized()) {
    ...
    int num_calc_footprint_cost, trajectory_range_check_obstacle_avoidance;
...
bool FixPatternTrajectoryPlannerROS::computeVelocityCommands(PlannerType planner_type, geometry_msgs::Twist* cmd_vel) {
  ...
  static int log_id = 0;
  geometry_msgs::PoseStamped robot_amcl_pose;
  tf::poseStampedTFToMsg(global_pose, robot_amcl_pose);
  double robot_amcl_orientation = tf::getYaw(robot_amcl_pose.pose.orientation);
    ...
    if (fabs(angle) <= yaw_goal_tolerance_) {
      ...
      if (planner_type == TRAJECTORY_PLANNER) {
        path = tc_->findBestPath(global_pose, traj_vel, highlight, current_point_dis,
                                            robot_vel, drive_cmds, &all_explored, log_id++);
      } else if (planner_type == LOOKAHEAD_PLANNER) {
      ...
      if (!rotating_to_goal_ && !fixpattern_local_planner::stopped(base_odom, rot_stopped_velocity_, trans_stopped_velocity_)) {
  ...
  Trajectory path;
  if (planner_type == TRAJECTORY_PLANNER) {
    path = tc_->findBestPath(global_pose, traj_vel, highlight, current_point_dis, robot_vel, drive_cmds, &all_explored, log_id++);
    // path = tc_->findBestPath(global_pose, 0.3, 0.5, current_point_dis, robot_vel, drive_cmds, &all_explored, log_id++);
  } else if (planner_type == LOOKAHEAD_PLANNER) {
    path = la_->GeneratePath(global_pose, robot_vel, traj_vel, highlight, &drive_cmds);
  }
